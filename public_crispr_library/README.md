...
- `restricted_library/` — `processed/` restricted to genes that are both (a) HGNC protein-coding and (b) have a RefSeq Select-tagged canonical transcript in T2T-CHM13. Produced by `scripts/data_preparation_scripts/build_restricted_library.Rmd`. This is the actual GuideRefine input used in this study.

| Library | Raw file | Processed file | Reformat script |
|---|---|---|---|
...
Reformat scripts live in [`scripts/data_preparation_scripts/`](../scripts/data_preparation_scripts/), not here.

`reformat_sgRNA_library.rmd` modifies an input sgRNA library to match GuideRefine's requirement (sgRNA, spacer, gene), auto-detecting the gene-symbol and sequence columns (via HGNC matching and column length) for standard libraries.

`reformat_Jacquere_library.Rmd` similar, but additionally resolves Jacquere's multi-gene `GENE1|GENE2` fields into a single gene per guide.
...
2. Reformat it into `processed/` as a 3-column TSV (`sgRNA`, `spacer`, `gene`, no header):
   ...
   - **Non-standard format** (e.g. multi-gene-per-row fields, merging multiple sources): write a dedicated script — see `reformat_Jacquere_library.Rmd` for an example (resolves `GENE1|GENE2`-style fields).
3. Run `scripts/data_preparation_scripts/build_restricted_library.Rmd` to regenerate `restricted_library/`.
